...
<html>
    ...
    <body class="fondo">
        ...
        <div class="container col-sm-4" style="border:double; border-radius: 5px;background-color: #17A2B8 ; border-color:black; opacity: 0.9">
            <h1 class="text-center"><span class="badge badge-info sombra">Nuevo Servicio al Registro</span></h1>
                <form class="form-horizontal" method="POST" action="/LAVARELA.110534/AgregarDetalleRegistroServlet">
                    <input type="text" name="txtIdDetalleRegistro"  value="${idDetalleRegistro}" hidden="" class="form-control mb-4"/>
                    <input type="text" name="txtIdRegistro" hidden=""  value="${idRegistro}"  placeholder="idRegistro" class="form-control mb-4"/>
                    <label><span class="sombra" style="color: white">Seleccione un Servicio</span></label>
                    <select class="form-control mb-4" name ="txtServicio" placeholder="Servicio"/>
                        <c:forEach items="${ListadoServicios}" var="x">
                            <option value="${x.idServicio}">${x.servicio}   $${x.precio} </option>
                        </c:forEach>
                    </select> 
                    <label><span class="sombra" style="color: white">Indique la cantidad deseada</span></label>
                    <input type="number" name="txtCantidad"  value="${cantidad}" min="1" max="200" placeholder="Cantidad" class="form-control mb-4" required=""/>     
                    <center><button type="submit" class="btn btn-light sombra text-center">Agregar</button></center>
                    <br>
                </form>
        </div>  
          <center>
